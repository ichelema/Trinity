...
function finish() {
  ...
  try {
    ...
    if (/^[/@$](trinity:)?ponytail/.test(prompt)) {
      const parts = prompt.split(/\s+/);
      const cmd = parts[0].replace(/^[@$]/, '/').replace(/^\/trinity:/, '/');
      const arg = parts[1] || '';
      ...
      if (cmd === '/ponytail-review' || cmd === '/ponytail:ponytail-review') {
        ...
        // valid default (#377), so only off/lite/full are accepted.
        if (arg === 'default') {
          const dmode = parts[2];
          if (dmode === 'off' || dmode === 'lite' || dmode === 'full') {
