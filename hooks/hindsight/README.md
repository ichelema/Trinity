...
| ---------------------------- | ------------------ | --------------------------------------------------------------------------------------- |
| `hindsight-recall.sh`        | UserPromptSubmit   | consuma il consenso del retain pending, valuta l'entry accodata allo Stop precedente (gate differito, ICH-86), poi esegue recall fresco, filtra i risultati e inietta solo memorie high o autorizzate; un solo JSON in output |
...
| `hindsight-retain-worker.py` | —                  | worker del retain: `evaluate_queued()` importato da `hindsight-recall.sh`, `--drain` dalla sentinella (non è un hook a sé) |
