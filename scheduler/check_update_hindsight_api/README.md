...
| `api-check-scheduled.cmd` | **Il ponte Windows→MSYS.** Lanciato da System Scheduler; imposta l'ambiente MSYS2 ed esegue lo script con `bash --noprofile --norc`.                                    |
...
System Scheduler (Splinterware)        ← innesco a orario programmato
        ...
        ▼
api-check-scheduled.cmd                 ← ponte: env MSYS2 + bash --noprofile --norc
        │
        ▼
...
| Application | `E:\AI\Claude\Trinity\scheduler\check_update_hindsight_api\api-check-scheduled.cmd` |
...
1. scrive `scheduler/check_update_hindsight_api/api-update-ALERT.txt` con il dettaglio installed/latest per pacchetto e il comando per aggiornare;
...
- `scheduler/check_update_hindsight_api/api-check-scheduled.log` — storico di ogni esecuzione (una riga per run, con JSON).
- `scheduler/check_update_hindsight_api/api-update-ALERT.txt` — presente **solo** quando c'è una versione da installare.
