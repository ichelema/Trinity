...
> upstream `muckybuzzwoo/claude-code-youtube-extract`. Dentro Trinity la skill è caricata con
> namespace `trinity:` (subagent `trinity:extract-worker`). I riferimenti a `CHANGELOG.md`,
> `tests/` e `requirements-dev.txt` puntano al **clone upstream** in
> `E:/AI/tools/claude-code-youtube-extract` (non presenti in questo repo).
