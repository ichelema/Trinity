...
**Autenticazione.** NON usare `notebooklm login` né `notebooklm auth refresh --browser-cookies`: richiedono un browser/Playwright o rookiepy, non disponibili qui. L'auth usa un file cookie già pronto (`E:/AI/tools/notebooklm-data/storage_state.json`). Se un comando fallisce per autenticazione scaduta, **FERMATI e avvisa l'utente**: deve rigenerare i cookie manualmente con i suoi 3 passi (in Chrome "Copia come cURL" → salva in `E:/AI/tools/notebooklm-data/curl.txt` → `ruby E:/AI/tools/notebooklm-data/make_storage_state.rb`). Claude non può effettuare il login da solo.

**Verifica auth (sola lettura, sicura):**
```bash
/e/AI/tools/notebooklm-data/notebooklm auth check --test --json   # richiede status=ok E checks.token_fetch=true
/e/AI/tools/notebooklm-data/notebooklm list --json
```
...
Nella cartella `references/` accanto a questa skill ci sono documenti dettagliati. **Non leggerli tutti** né caricarli preventivamente: apri solo quello pertinente quando la documentazione in questo file non basta.

| File | Leggilo quando |
|------|----------------|
| `references/cli-reference.md` | Ti serve la sintassi esatta, le opzioni o gli exit code di un comando `notebooklm` (reference CLI completo). |
| `references/python-api.md` | Stai usando notebooklm come **libreria Python** (uso programmatico async), non da riga di comando. |
| `references/architecture.md` | Devi capire la struttura interna / il layering (CLI, MCP, REST, RPC), es. per estendere o debuggare in profondità. |
| `references/rpc-reference.md` | Ti servono i dettagli di basso livello: metodi RPC, selettori UI, struttura dei payload degli artifact. |
| `references/mcp-guide.md` | Stai configurando o usando il **server MCP** di notebooklm (adapter sperimentale, 25 tool). |
